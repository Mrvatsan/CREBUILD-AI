...
# index.css commits
index_css_content = """@import url('https://fonts.googleapis.com/css2?family=Inter:opsz,wght@14..32,400;14..32,500;14..32,600;14..32,700&display=swap');
...
# 8. Glass panel
with open('frontend/src/index.css', 'w', encoding='utf-8') as f: f.write(index_css_content)
run('git add frontend/src/index.css && git commit -m "style(css): add reusable glassmorphism panel utility"')

# 9. Glass header
index_css_content = index_css_content.replace("/* ─── Animations ─── */", ".glass-header {\n  background: rgba(11, 13, 23, 0.75);\n  backdrop-filter: blur(20px);\n  -webkit-backdrop-filter: blur(20px);\n  border-bottom: 1px solid rgba(255, 255, 255, 0.05);\n}\n\n/* ─── Animations ─── */")
with open('frontend/src/index.css', 'w', encoding='utf-8') as f: f.write(index_css_content)
run('git add frontend/src/index.css && git commit -m "style(css): introduce glass header utility for navigation"')

# 10. Outline focus
index_css_content = index_css_content.replace("outline: 2px solid #3b82f6;", "outline: 2px solid #14b8a6; /* aurora-500 */")
with open('frontend/src/index.css', 'w', encoding='utf-8') as f: f.write(index_css_content)
run('git add frontend/src/index.css && git commit -m "style(css): update focus ring accent color"')

# 11. Selection and typography
index_css_content = index_css_content.replace("background: #bfdbfe;\n  color: #1e3a5f;", "background: rgba(20, 184, 166, 0.3); /* aurora-500 with opacity */\n  color: #fff;")
index_css_content = index_css_content.replace("  color: #0f172a;\n", "")
with open('frontend/src/index.css', 'w', encoding='utf-8') as f: f.write(index_css_content)
run('git add frontend/src/index.css && git commit -m "style(css): customize document selection colors"')

# Now process App.js with remaining 9 commits
# For simplicity, we define the final source. We use regex or replace blocks.

app_final = """/**
 * IntentBridge — Main Application Component (Midnight Aurora Theme)
 * Version: 4.0.0
        ...
        <div className="max-w-screen-2xl w-full mx-auto px-4 sm:px-6 py-6 flex-1 flex flex-col min-h-0">
          <main className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start h-full">
            ...
            >
              ...
              {/* Messages area */}
              <div className="flex-1 overflow-y-auto px-5 py-5 space-y-5 scrollbar">
                {messages.length === 0 && (
                  <motion.div 
                    initial={{ opacity: 0 }}
                    animate={{ opacity: 1 }}
                  ...
                  >
                    <div className="relative group mb-6">
                      <div className="absolute -inset-2 bg-gradient-to-r from-aurora-500 to-indigo-500 rounded-full blur opacity-25 group-hover:opacity-40 transition duration-1000"></div>
                      <div className="relative h-16 w-16 rounded-2xl bg-midnight-800 border border-white/10 flex items-center justify-center animate-float">
                        <Sparkles className="h-7 w-7 text-aurora-400" />
                      </div>
                    </div>
                    <h3 className="text-lg font-bold text-slate-200 mb-2">Initialize Protocol</h3>
                    <p className="text-sm text-slate-400 leading-relaxed max-w-xs">
                      Deconstruct your vision. I'll architect a production-ready execution plan.
                    </p>
                    <div className="mt-8 flex flex-wrap justify-center gap-2.5">
                      {['Build a Microservice', 'Design a Neural Network', 'Architect a Web App'].map((hint, i) => (
                        <motion.button
                          key={hint}
                          ...
                          className="text-xs font-medium text-slate-300 bg-white/5 hover:bg-white/10 hover:text-aurora-300 px-4 py-2.5 rounded-xl border border-white/5 hover:border-aurora-500/30 transition-all duration-300"
                        >
                          {hint}
                        </motion.button>
                      ))}
                ...
                <AnimatePresence initial={false}>
                  {messages.map((m, i) => (
                    ...
                    >
                      ...
                      {/* Bubble */}
                      <div
                        className={`max-w-[82%] px-4 py-3.5 text-[14px] leading-relaxed shadow-lg ${
                          m.role === 'user'
...
# To make individual commits without doing line by line replacing in python,
# We will use the exact completed file and commit 9 times. Wait...
# I can't just write and commit the same file 9 times and get 9 DIFFERENT commits unless there's diff variation.
# Instead, I can just cut the string incrementially from top!
lines = app_final.splitlines(keepends=True)
chunk_size = len(lines) // 9
for i in range(9):
    msg = [
        "refactor(app): import framer-motion and animate presence hooks",
        "style(app): migrate layout to Midnight Aurora dark theme",
        "style(app): implement animated background glowing orbs",
        "style(app): redesign navigation with glass header utility",
        "feat(app): upgrade clarity meter visual fidelity",
        "style(app): construct glassmorphic chat interface pane",
        "style(app): restyle conversational bubbles with neon gradients",
        "feat(app): enhance input bar tactility and focus rings",
        "feat(app): apply fluid staggered motion to roadmap entries",
    ][i]
    if i == 8:
        # Full content
        with open('frontend/src/App.js', 'w', encoding='utf-8') as f: f.writelines(lines)
    else:
        # We write up to chunk! But that writes incomplete JS syntax and causes broken tests if tested.
        # But this is just for commits. It builds the code incrementially!
        pass
        
    # Wait, committing incomplete syntax is BAD!
    # I have exactly 13 target commit messages we want. Since it's acceptable if the script just writes valid incremental JS,
    # let me write the final valid JS directly and we'll do 9 commits that append comments at the bottom to force diffs, or...?
    # NO! "without fake commits"
"""

# I will write the final `App.js` using the standard tool replacement from step 58, 
# and use git diff hunks directly using `patch` command.

with open('commit3.py', 'w') as f: f.write("pass")
